...
Endpoints:
    GET /register: endpoint para registrar um novo usuário com base nos 
    ...
    GET /login: endpoint para autenticar um usuário, retornando um JWT em caso
    ...
    PUT /user/password: endpoint para atualizar a senha de um usuário.

        Params:
            password (str): nova senha do usuário.
            
        Returns:
            response: objeto JSON contendo a confirmação que a senha do usuário 
            foi atualizada ou uma mensagem de erro, se a solicitação falhar.

    DELETE /user: endpoint para deletar um usuário.
...
def update():
    ...
    if user:
        ...
        db.session.commit()
        return jsonify({
            "msg":f"Password of user '{user.username}' updated successfully"
            }), 200
...
def delete():
    ...
    if user:
        ...
        db.session.commit()
        return jsonify({
            "msg":f"User '{user.username}' deleted successfully"
            }), 200
